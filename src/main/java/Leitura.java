...
public class Leitura {
    ...
    public static void tamString(File file, String nomeArqSaida, int atributo) throws IOException {
        ...
        int quantAtributos = 0, progresso = 0, progressoAtual = 0;
        ...
        while ((linha = sc.readLine()) != null) {
            ...
            for (int i = 0; i < linha.length(); i++) {
                ...
                if (i == linha.length() - 1 && linha.charAt(i) != '\"') {
                    linha = sc.readLine();
                    while (linha.length() == 0) // Caso a proxima linha seja apenas um espaço em branco encontrar uma
                                                // linha que contenha alguma informção
                        linha = sc.readLine();
                    i = -1;
                } else {
                    if (i < linha.length() - 2) // Não se deve fazer a proxima comparação caso o caractere seja o
                        ...
                        if (linha.charAt(i) == '\"' && linha.charAt(i + 1) == ',' && linha.charAt(i + 2) == '\"') { // Verifica
                            ...
                            if (aux.matches("\".*\"") || quantAtributos == 3) { //
                                ...
                                if (quantAtributos == atributo && description.equals("")) { // Caso o atributo seja o
                                                                                            // escolhido ao chamar
                                                                                            // função
